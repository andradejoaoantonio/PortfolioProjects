...
-- Creating the REGIONS table and inserting VALUES into it
...
-- Creating the EMPLOYEES table and adding VALUES to it
...
-- Assignment #1
-- Create the database "public" and the following tables:
...
## 1) What is the datatype of the course_no column?

SELECT column_name, data_type
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'teach';

/*2) How many columns does the student_enrollment table have?*/
...
/*3) How many rows does the teach table have?*/

SELECT COUNT(*) FROM teach;

-- SQL Query Basics
/*Check only the employees that work in the sports department*/
...
SELECT * FROM employees
WHERE department LIKE 'S%or%s'
ORDER BY employee_id; 

/*Check the M employees which salary is >= 10000, ordering by salary ascending */
SELECT *
FROM employees
WHERE salary >= 10000
AND gender = 'M'
...
/*Check the total number of employees and their average salary, grouped by gender*/
SELECT gender,COUNT(gender) AS total_employees, ROUND(AVG(salary),2) AS salary_average
FROM employees
GROUP BY gender
ORDER BY ROUND(AVG(salary),2) DESC;

/*Check the total number of employees and their average salary, by region and per gender*/
SELECT region_id, gender,COUNT(gender) AS total_employees, ROUND(AVG(salary),2) AS salary_average
FROM employees
GROUP BY region_id, gender
ORDER BY region_id, gender, total_employees;
...
CREATE OR REPLACE VIEW sal_between_10_15k AS
	SELECT COUNT(*) FROM employees
	WHERE salary BETWEEN 100000 AND 150000;

/*Total number of employees, total number of employees who earn between 10-15k and their percentage*/
SELECT
	COUNT(*) AS total_employees,
	(SELECT COUNT(*)
 		FROM employees
		WHERE salary BETWEEN 100000 AND 150000)
			AS sal_between_10_15k,
		CAST((SELECT * FROM sal_between_10_15k)/(SELECT COUNT(*)
	FROM employees) AS DOUBLE PRECISION)  
FROM employees;
...
SELECT
	...
	(SELECT COUNT(*)
		...
		WHERE salary BETWEEN 100000 AND 150000)
			AS sal_between_10_15k,
		ROUND((SELECT * FROM sal_between_10_15k)/(SELECT COUNT(*) FROM employees),2)  
FROM employees;

/*Return the first_name and email of females that work in the tools department,
...
/*Return the first_name and hire_date of those employees who earn more than 165000
...
/*Return the first names and hire dates of those employees who where hired
during january 1st 2002 and jan 1st 2004*/

SELECT first_name, hire_date FROM employees
WHERE hire_date BETWEEN '01-01-2002' AND '01-01-2004'
ORDER BY first_name;

/*Return the male employees who work in the automotive department and earn more
than 40000 and less than 100000 as well as females that work in the toys department*/
...
-- ORDER BY, LIMIT / FETCH FIRST, DISTINCT & Renaming Columns

SELECT DISTINCT department FROM employees
ORDER BY 1
FETCH FIRST 10 ROWS ONLY; -- supported in some versions only

SELECT DISTINCT department FROM employees
ORDER BY 1
LIMIT 10;

SELECT DISTINCT department AS sorted_departments FROM employees
ORDER BY 1
FETCH FIRST 3 ROWS ONLY; -- supported in some versions only
...
/* 2.Write a query to display all of those students that contain the letters "ch" in their name
...
/* 3.Write a query to display the name of those students that have the letters "ae" or "ph"
...
/* ADVANCED: 6. Write a query that returns students based on the following criteria:
...
SELECT UPPER(first_name), LENGTH(first_name), LOWER(department)
FROM employees;
...
/*A boolean is something like "true" or "false"
A boolean expression is written between (), for example, "(salary > 140000)", as shown below*/

SELECT * FROM employees;

SELECT first_name || ' ' || last_name AS full_name, (salary > 140000) department FROM employees;

/*What if we want to show only the true values?*/

SELECT first_name || ' ' || last_name AS full_name, (salary > 140000) AS is_higly_paid, department FROM employees
WHERE (SALARY > 140000) = 'true';

/*Boolean expression example
Boolean expression is case sensitive*/
...
SELECT SUBSTRING('This is test data' FROM 1 FOR 7) AS test_data_extracted;

SELECT SUBSTRING('This is test data' FROM 7) AS test_data_extracted;

SELECT department FROM departments;

SELECT department, REPLACE (department,'Clothing','Attire') AS mod_data
FROM departments;

/*What if we want to add the word "department" to every department namde?*/

SELECT department, department || ' ' || 'Department' AS full_department_name
FROM departments;
...
/*What if we want the domain without the @?*/
SELECT SUBSTRING(email,POSITION('@' IN email)+1)
FROM employees;

SELECT email, SUBSTRING(email,POSITION('@' IN email)+1) AS formated_text
FROM employees;

-- COALESCE
/* COALESCE function allows to replace a NULL values for non-null value*/
...
SELECT
	...
	(COUNT(employee_id) - COUNT(email)) AS missing_emails
FROM employees;

-- SUM

SELECT region_id, department, SUM(salary) AS salary_budget
FROM employees
GROUP BY region_id, department
ORDER BY region_id, department, salary_budget DESC;
...
SELECT last_name || ' ' || 'works in the ' || department || ' department' AS question_1
FROM professors
...
/* 6.Write a query that will display the table professors, together with the total working days and years,
ordering from highest to lowest working time since hire date*/

SELECT *, (CURRENT_DATE - hire_date) AS total_working_days,  ((CURRENT_DATE - hire_date)/365) AS total_working_years
FROM professors
...
/* What is the total count of employees working for each department?*/

SELECT department, COUNT(employee_id) AS nº_employees
FROM employees
GROUP BY department
ORDER BY nº_employees DESC;

SELECT department, COUNT(*) AS total_number_employees, ROUND(AVG(salary)) AS avg_sal, MIN(salary) AS min_sal, MAX(salary) AS max_sal
FROM employees
WHERE salary > 70000
GROUP BY department
ORDER BY total_number_employees DESC;

/*What if we wanted to know the total number of employees
by department and gender?*/

SELECT department, gender, COUNT(*) AS total_number_employees
FROM employees
GROUP BY department, gender
ORDER BY department;

/*To filter aggregated data, we should use the HAVING clause*/
...
/* Number of employees with the same first name, showing those names and their counts*/

SELECT first_name, COUNT(first_name) AS same_first_name
FROM employees
GROUP BY first_name
HAVING COUNT(first_name) > 2
ORDER BY same_first_name DESC;
...
/* Name of the unique set of departments*/
...
/* Different domain names in our company and the total number of employees that have that given domain name.*/

SELECT SUBSTRING(email,POSITION('@' IN email)+1) AS email_domain, employee_id
FROM employees
WHERE email IS NOT NULL
GROUP BY email_domain, employee_id
...
/* Report for the average, highest and lowest salary per region and by gender*/
...
-- Assignemnt #4: Aggregation Queries
...
SELECT state, COUNT(name) AS fruit_types
FROM fruit_imports
GROUP BY state
HAVING COUNT(name) > 1
ORDER BY fruit_types DESC
LIMIT 1;
...
Execute the below SQL script to answer to question 6:
...
SELECT COUNT(COALESCE(fruit_name,'Fruit'))
FROM fruits;

-- Using Subqueries
-- Aliasing Souces of Data
...
/* We can name the tables and reduce the query writing:*/

SELECT emp.department
FROM employees AS emp, departments AS dept;

-- SUBQUERIES
-- All the departments that exist on the "employees" and NOT in the "departments" table
...
-- For the below query to work, we need to use an alias because it is a "source",
...
/*In case you want to use aliases, we need to make the aliases the same
inside and outside the subquery*/
...
/* When you now have assigned names/aliases to the "sources", you need to make sure to which columns
you are referring to in the main query and that these are not repeated. If you have repeated column names,
make sure that you are addressing the correct one*/

SELECT a.employee_name, yearly_salary
FROM (SELECT first_name AS employee_name, salary AS yearly_salary
	  FROM employees WHERE salary > 150000) AS a,
	  (SELECT department AS employee_name FROM departments) AS b;
	  
/* The rule of thumb is that if you are using aliases, we have to source of data.
If you're giving your sources of data aliases always reference to aliases.
It's easier when writing/reading your queries.*/
...
/*Once you use a subquet in a FROM clause, you have to give it an alias*/
...
/*So the assignment is give me a very select statement that returns all of those employees that work in
the electronics division. Notice I use the word division. I'm not saying the electronics department.
I'm saying the electronics division.*/
...
SELECT department, COUNT(employee_ID) AS number_employees FROM employees
WHERE department IN (SELECT department FROM departments WHERE division = 'Electronics')
GROUP BY department
ORDER BY number_employees DESC;


/* Which are the employees that work in Asia or Canada, and make over 130000 dollars?*/
...
/* What are the first name and the department that an employee works for, and how much less they make than
the highest paid employee in the company?. These employees should be working in Asia and Canada.*/

SELECT first_name, department, salary,
	(SELECT MAX(salary) FROM employees LIMIT 1) AS max_salary,
	(salary-(SELECT MAX(salary) FROM employees LIMIT 1)) AS diff_salary
FROM employees
...
/* In the following query, using only the operators such as "=" and ">" will not work
because it only works when you are searching for a single value, not values within a source.
We must use also the ANY or ALL clause*/

SELECT * FROM employees
WHERE region_id = (SELECT region_id FROM regions WHERE country = 'United States');
...
and the dates which those employees were hired is greater than all of the hire_dates of employees
...
/* Employees that make the same exact salary value.
In mathematics, this is known as MODE, which is the element that appears most frequently for a give set of data*/
...
/* Let's create a table with duplicates so we can exercise a bit more with subqueries*/
...
/* How can we get only the lines with distinct names */
...
/* Let's delete the table dupes using the DROP TABLE clause*/

DROP TABLE IF EXISTS dupes;


/* When collecting data, we have outliers, data that throws off the analysis because, in statistics,
an outlier is a data point that differs significantly from other observations, due to a variability
in the measurement, an indication of novel data, or a result of experimental error.*/

/* What is the average salary, excluding the the highest and the lowest paid salary employees.*/
...
/* When running the above queries, we can see that the students table is not directly related to the courses table.
The "students" table contains only student details, while the "courses" table contains only the courses information.
The table that relates both the "students" table and "courses" table is the "student_enrollment" table, which has 
the details regarding which student is enrolled in which course.*/
...
WHERE course_no = ANY (SELECT course_no FROM courses
WHERE course_title IN ('Physics', 'US History'));
...
SELECT course_no, COUNT(course_no)
FROM student_enrollment
GROUP BY course_no;

SELECT student_no, COUNT(course_no)
FROM STUDENT_ENROLLMENT
GROUP BY student_no
...
FROM (SELECT student_no, COUNT(course_no)
	  ...
	  GROUP BY student_no
	  ORDER BY COUNT(course_no) DESC
	  LIMIT 1) AS student_no_highest_courses;
...
/* 5. Write a query to find the student that is the oldest without using the LIMIT or the ORDER BY clause to solve this problem.*/
...
-- Conditional Expressions Using CASE + exercises

SELECT first_name, salary,
CASE
	WHEN salary < 100000 THEN 'Underpaid'
	WHEN salary > 100000 AND salary < 160000 THEN 'Paid Well'
	WHEN salary > 160000 THEN 'Executive Paid'
	ELSE 'Unpaid'
END AS salary_category
FROM employees
ORDER BY salary DESC;

/* What is the total counts of the following categories: "Underpaid", "Paid Well", and "Executive Paid".*/

SELECT category.salary_category, COUNT(*) FROM (
SELECT first_name, salary,
CASE
	WHEN salary < 100000 THEN 'Underpaid'
	WHEN salary > 100000 AND salary < 160000 THEN 'Paid Well'
...
/* We can also use numerical categories instead of text*/
...
/* What is the total count of employees per department?
What if we wanted to have the data tranposed?*/
...
/* Employees broken-down by regions (transposed)*/
...
/*first, we must get the total supply for each fruit,
this is the source we will use to categorize, so , we must name it
in the final query*/
...
/* Next step, lets use CASE to categorize the supply as LOW, ENOUGH and FULL
and add the source from above*/
...
/* Taking into consideration the supply column and the cost_per_unit column, tabulate the total cost
to import fruits by each season. The result shuld look something like this:
...
/*Now, let's transpose the above data...*/
...
/* A correlated subquery basically means that the subquery portion is correlated with the outer query,
it is a query nested inside of another that uses values from the outer query.*/

/* Employees that make a salary higher than their department salary*/

-- NOTE: E1 is the outer employee table while E2 is the inner employee table
...
/* Name of departments that have more than 38 employees working.*/
...
-- Let's add another column with the number of employees of each department that has more than 38 employees...

SELECT department, COUNT(employee_id) AS number_employees
FROM employees
GROUP BY department
HAVING COUNT(employee_id) > 38;
...
-- What if we would use the "employees" table?
...
/* Name of departments that have more than 38 employees working and
the highest paid employees salary for each one of these departments.*/
...
SELECT department, (SELECT MAX(salary) FROM employees WHERE department = d.department) AS number_employees
FROM departments d
WHERE 38 < (SELECT COUNT(*)
		   FROM employees e2
...
-- Assignment Correlated Subqueries

SELECT * FROM departments;
SELECT * FROM employees;

/* The the department and the first_name of the employees with the 
overall highest and lowest salary in the dataset */
...
/* List of employees showing the highest and lowest paid employee per department
First, let's make a table showing the departments, first_name, MAX and the MIN salaries and name it as "a"*/
...
/* Next step, let's add a WHERE clause to show only the MAX and the MIN salaries
per department in the same column*/
...
/* Coutry where the employee works*/

SELECT first_name, country
FROM employees, regions
WHERE employees.region_id = regions.region_id;

/* Employee's names, their email address, and the division in which that employee works,
excluding the ones where the email is not available*/
...
/* Adding the country and department as well to the above query...*/
...
/* Let's simplify using alias...*/

SELECT first_name, email, emp.department, division,country
FROM employees AS emp, departments AS dept, regions AS reg
WHERE emp.department = dept.department
AND emp.region_id = reg.region_id
AND email IS NOT null
ORDER BY country;


/* Country and total number of employees that work in that country */

SELECT country, COUNT(*) AS number_employees
FROM regions AS reg, employees AS emp
WHERE reg.region_id = emp.region_id
GROUP BY country
ORDER BY number_employees DESC;

SELECT country, COUNT(*) AS number_employees
FROM (SELECT * FROM regions) AS reg, employees AS emp
WHERE reg.region_id = emp.region_id
GROUP BY country
ORDER BY number_employees DESC;


-- INNER & OUTER JOINS + Exercises
...
/* Name of employee and the country they work in */
...
/* Employees contacts and working division */
...
/* Employees contacts, working division and country.
By adding a second JOIN, we are joining on the results of the 
first JOIN and so on*/
...
/* Employees assigned to departments that do not exist in departments table. */
...
/* The below query is a LEFT JOIN on the "department" column in both tables, but the LEFT JOIN 
means that it will match all of the departments that exist in the "employees" table, regardless
of whether or not they exist in the department's table. It will show NULL in the RIGHT table if it exists
 in the LEFT table and not in the table on the RIGHT */
...
/* The RIGHT JOIN means that it will match all of the departments that exist in the "departments" table
(on the RIGHT of the statement), regardless of whether or not they exist in the "employees" table. It will show
NULL in the LEFT table if it exists in the RIGHT table and not in the table on the LEFT  */
...
/* The FULL OUTER JOIN keyword returns all records when there is a match in left (employees) or right (departments) table records.
NOTE: The FULL OUTER JOIN and FULL JOIN are the same. */

/* The deparment that shows NULL is the department that exists in the "departments" table
...
-- While UNION only returns unique, 
-- UNION ALL returns all records, including duplicates.

-- The SQL EXCEPT statement returns those records from the left/above SELECT query,
-- that are not present in the results returned by the SELECT query on the right/bottom side of the EXCEPT statement.
...
/* Total of employees working in each deparment and the total of employees*/
...
/* The table "employees" has 1000 rows, while the table
"departments" has 24, so, if we SELECT COUNT from both of them,
we will get 2400 = 1000 * 24

This is called a Cartesion Product

When we add sources in the FROM clause, we are increasing the source

We can achieve a Cartesian Product without JOINS or using a CROSS JOIN,
which is a more explicit way to say we need a Cartesian Product*/

SELECT * FROM departments;
SELECT * FROM employees;

SELECT COUNT(*) FROM (
	SELECT * FROM employees, departments) AS a;
...
SELECT COUNT(*) FROM (
	SELECT FROM employees AS A, employees AS B,departments
	) AS sub;
...
/* First name, department, hire date, and the country of the first and last hired employee */

(SELECT first_name, department, hire_date, reg.country FROM employees AS emp
...
/* When using LIMIT, we should place the queries in parenthesis.
Another example of the above would be as follow... */

(SELECT first_name, department, hire_date, country
FROM employees AS emp
INNER JOIN regions AS reg
ON emp.region_id = reg.region_id
WHERE hire_date = (SELECT MIN(hire_date) FROM employees AS emp2)
LIMIT 1)
UNION
SELECT first_name, department, hire_date, country
FROM employees AS emp
INNER JOIN regions AS reg
ON emp.region_id = reg.region_id
WHERE hire_date = (SELECT MAX(hire_date) FROM employees AS emp2)
ORDER BY hire_date


/* Employee salary spending fluctuation for every 90 day period, cumputing the sum of salaries for every 90 days,
starting with the date of the first employee all the way down to the last employee that we hired. */
...
SELECT
	hire_date,
	salary,
	(SELECT SUM(salary) FROM employees AS emp2
		WHERE emp2.hire_date BETWEEN (emp.hire_date - 90) AND emp.hire_date) AS spending_pattern
FROM employees AS emp
ORDER BY hire_date;


-- NOTE: A correlated subquery uses information from the outer query
-- and it run for every single record of the outer query

-- Creating VIEWS (or HARD VIEW) vs INLINE VIEWS
-- Note that VIEW is a query, not a table, but it treats as a table
-- when querying
...
/* Employees' first name, division and country they work in*/

CREATE VIEW view_employee_information AS
SELECT first_name, email, emp.department, salary, division, region, country
FROM employees AS emp, departments AS dep, regions AS reg
WHERE emp.department = dep.department
AND emp.region_id = reg.region_id;

SELECT * FROM view_employee_information;

SELECT first_name, region
FROM view_employee_information;

-- INLINE VIEW

SELECT *
FROM (SELECT * FROM departments) AS inline_view;
...
SELECT * FROM student_enrollment;
SELECT * FROM professors;
SELECT * FROM professors, student_enrollment;

-- They are NOT directly related, because there is no common column
-- shared amongst then, hence, there cannot be a direct relationship formed between them
-- We would either need to create a new column with common data or, f.e., add the "course_no" data to to 
-- professors table so we could create a relationship between them
...
SELECT students.student_no, student_name, course_title, courses.course_no, last_name AS professor
FROM student_enrollment
INNER JOIN courses ON student_enrollment.course_no = courses.course_no
INNER JOIN students ON student_enrollment.student_no = students.student_no
INNER JOIN teach ON courses.course_no = teach.course_no
ORDER BY students.student_no;
...
-- The combination of student_name and course_no is being repeated for as many professors that are teaching
-- that particular course. If you ORDER BY the student_name column, you'll clearly be able to see that
-- multiple professors are teaching the same subject.
-- For example, course CS110 is being taught by both Brown and Wilson.
-- That is why you'll see the combination of the student Arnold with CS110 twice.

/* 4. In question 3 you discovered why there is repeating data. How can we eliminate this redundancy?
Let's say we only care to see a single professor teaching a course and we don't care for all the other professors that teach
the particular course. Write a query that will accomplish this so that every record is distinct.
HINT: Using the DISTINCT keyword will not help. :-) */

SELECT students.student_no, student_name, course_title, last_name AS professor
FROM student_enrollment
INNER JOIN courses ON student_enrollment.course_no = courses.course_no
INNER JOIN students ON student_enrollment.student_no = students.student_no
INNER JOIN teach ON courses.course_no = teach.course_no
...
FROM (
	...
	FROM students
	INNER JOIN student_enrollment ON students.student_no = students.student_no
	INNER JOIN teach ON student_enrollment.course_no = teach.course_no
	INNER JOIN professors ON teach.last_name = professors.last_name
    ) a
...
-- the rows of the containing/outer query) will execute once for each row.
-- A non-correlated subquery (one in which the where condition is independent of the containing query)
-- will execute once at the beginning. If a subquery needs to run for each row of the outer query, that's going be very slow!
...
FROM students AS s
LEFT JOIN student_enrollment AS se
...
-- OVER clause
-- Like aggregation functions, window functions perform calculations based on a set of records, 
-- e.g. finding the average salary across a group of employees.
...
-- Using the OVER clause empty, we will get the total number of rows/employees

SELECT first_name, department, COUNT(*) OVER() AS total_employees
FROM employees;

-- If we use the PARTITION BY inside the OVER cause,
-- stating which data to aggregate, we can get a 

SELECT first_name, department, COUNT(*) OVER(PARTITION BY department) AS total_depart_employees
FROM employees;

-- To prove this, let's use the EXCEPT clause to see
-- that the data is the same in both following queries,
...
-- Sum of all the salary for each department

SELECT first_name, department, SUM(salary) OVER(PARTITION BY department)
FROM employees;

SELECT first_name, department, SUM(salary) OVER()
FROM employees;

-- Number of employees that work in each department and
-- number of employees that work in each region
...
-- Using the COUNT all and the OVER without partitioning,
-- we will get the total count of the database and,
-- if we add a WHERE clause, the total will be the filter
-- that results from that WHERE clause
...
-- How to display the running total? Using the Window Function form above
...
SELECT
	...
	SUM(salary) OVER(ORDER BY hire_date
					 RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_total_salaries
FROM employees
ORDER BY hire_date;

SELECT employee_id, first_name, hire_date, salary, SUM(salary) OVER(ORDER BY hire_date)
FROM employees
ORDER BY hire_date;
...
-- Running total of employees over time
...
SELECT
	...
	) AS running_total_employees
FROM employees
ORDER BY hire_date;

SELECT employee_id, first_name, hire_date, COUNT(*) OVER(ORDER BY hire_date)
FROM employees
ORDER BY hire_date;

-- Adding adjacent salaries
...
/* Total salary example using window frames */
/* Total salary = 91571594 */
...
-- Ranking and Bucketing Data Using Window Functions
-- RANK, FIRST_VALUE, NTILE functions

-- Ranking salaries from highest to lowest, by department
...
-- Employees which rank as "8" on the salary ranking, by department
...
-- Creating buckets of data for the salary ranking

SELECT first_name, email, department, salary, NTILE (5) OVER(PARTITION BY department ORDER BY salary DESC) AS salary_bracket
FROM employees;
...
SELECT first_name, email, department, salary,
	FIRST_VALUE(salary) OVER(PARTITION BY department ORDER BY salary DESC) AS first_value_salary
FROM employees;


/*Let's compare with the MAX function, we can see they are getting the same values,
but the advantage of the FIRST_VALUE is that we can ORDER BY any other column,
which using the MAX funciton does not allow us*/
...
-- Let's get the 5th employee,
-- so, basically, the 5th employee salary will be repeated, while
-- the salaries higher than the 5th will display as null

SELECT first_name, email, department, salary,
	NTH_VALUE(salary,5) OVER(PARTITION BY department ORDER BY first_name) AS nth_value
FROM employees;


-- Working with LEAD and LAG functions
/* LEAD and LAG allows us to get the values from a row that is directly above or below the currently
processing row.*/
...
/* For every row, we want to see the salary of the row that comes directly after,
so we should use the LEAD function */
...
/* To see the salary of the row that comes directly before,
we then will use the LAG function */
...
/* A good use of this function is if you want to have side by side
comparison of an employee's salary and the salary amount directly above
that employee.

Same thing if you need to check side by side but comparing the 
salary directly below*/

SELECT department, last_name, salary,
	LAG(salary) OVER(ORDER BY salary DESC) AS closest_higher_salar
FROM employees;
...
-- Sorting salaries from high to low partitioned by department:
...
/* Breakdown of each one of these groups using a single query */

SELECT continent, country, city, SUM(units_sold)
FROM sales
GROUP BY GROUPING SETS(continent, country, city);

-- adding the total to the first row...
...
-- ROLLUP
-- Sum of units sold if we were to group by all of these columns.
...
-- CUBE
-- CUBE is a subclause of the GROUP BY clause, allows you to generate multiple grouping sets and, based on the input,
-- it will display all types of combinations for the SUM of units_sold
-- continent + country + city, or
...
------------------------------------------------------------------------------------------------
/*In this section you'll be working with the tables you created in assignment 1.
The problems in this section are challenging! Before starting these problems, you should have solved all
of the previous assignments in the course. Getting the questions right in this section means you're a SQL Expert!

The questions in this section will require you to use the tables you created in assignment 1. Those tables are:
...
/* Option 2, showing the name of the students and considering the course CS180 "1"
when it shows up and returning 0 for that course...*/
...
HAVING MAX(CASE
		   	WHEN se.course_no = 'CS180' THEN 1 ELSE 0
		   END) = 0;
...
SELECT *,
	CASE
		WHEN (course_no = 'CS110' OR course_no = 'CS107') THEN 1
		ELSE 0
	END AS course_detection
FROM student_enrollment;

SELECT
	*,
	CASE
		WHEN course_no = 'CS110' THEN 'CS110'
	 	WHEN course_no = 'CS107' THEN 'CS107'
		ELSE NULL
	END AS course_detection
FROM student_enrollment;
...
SELECT
	*,
	CASE
		WHEN (student_no = 1) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
		WHEN (student_no = 2) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
		WHEN (student_no = 3) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
		WHEN (student_no = 4) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
		WHEN (student_no = 5) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
		WHEN (student_no = 6) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
		WHEN (student_no = 7) AND (course_no = 'CS110') AND (course_no = 'CS107') THEN 'Both'
	ELSE '00'
END AS course_detection
FROM student_enrollment;

-- Solution A
/* Solution A uses a self join on the student_enrollment table so that those students are narrowed down that take
both CS110 and CS107 in the subquery. The outer query filters for those student_no that are not the ones retrieved from the subquery. */
...
-- Solution B
/* In solution B, a CASE expression is used with the aggregate SUM function to find students
who take either CS110 or CS107, but not both. */
...
SELECT student_name, course_no, COUNT(course_no)
FROM student_enrollment
INNER JOIN students ON student_enrollment.student_no = students.student_no
...
So the student_no from the inline view is joined with the outer query and we filter for
only those students that take the course CS220. So this query returns that one student that takes CS220 and no other course. */
...
SELECT se.student_no, student_name
FROM student_enrollment AS se
INNER JOIN students AS s
ON se.student_no = s.student_no
GROUP BY se.student_no, student_name
