-- For this analysis, the following tables are going to be created manually
-- DEPARTMENTS, REGIONS, EMPLOYEES
-- STUDENTS, STUDENT_ENROLLMENT, PROFESSORS, TEACH, COURSES
-- FRUIT_IMPORTS, FRUITS
-- SALES
-- You can follow this analysis and check the data by using the queries used in this analysis
...
-- Checking the datatype of the course_no column
...
-- Students that do not take CS180
-- Option A
SELECT * FROM students
...
-- Students that take CS110 or CS107 but not both courses
-- Solution A
...
-- Students who take only CS220 and no other courses
-- Solution A
SELECT s.*
FROM students s, student_enrollment se
WHERE s.student_no = se.student_no
...
-- Students who take at most 2 courses, excluding students that don't take any courses
-- as well as those that take more than 2 course
-- Solution A
...
-- Students who are older than at most two other students
SELECT s1.*
FROM students s1
...
-- Number of employees that work in the sports department
...
-- Male employees with salary higher or equal to 10000, ordered by salary in ascending order
...
-- Total number of employees and their average salary, grouped by gender
...
-- Total number of employees and their average salary, by region and gender
...
-- Employees that do not have an email

SELECT * FROM employees
WHERE email IS NULL

-- Number of employees that have a range salary between 100000 and 150000
...
-- For this step, a view was created to simplify the query
...
SELECT
	...
	(SELECT COUNT(*)
 	FROM employees
	WHERE salary BETWEEN 100000 AND 150000) AS sal_between_10_15k,
	(ROUND((SELECT * FROM sal_between_10_15k)/(SELECT COUNT(*)::DECIMAL FROM employees),2)*100) AS percentage
FROM employees;


-- first_name and email of female workers that work in the tools department,
-- having a salary greater than 110000 and email
...
-- First name, hire date and working time of employees who earn more than 165000
-- as well as male employees that work in the sports department

SELECT
	first_name,
	hire_date,
	ROUND((CURRENT_DATE - hire_date)/365::DECIMAL,2) AS working_time_years
FROM employees
WHERE salary > 165000 OR (department = 'Sports' AND gender = 'M')
ORDER BY working_time_years DESC;


-- First names and hire dates of those employees who where hired
-- between January 1st 2002 and January 1st 2004
...
-- Male employees who work in the automotive department and earn more
...
-- Names of those students that are between the ages of 18 and 20.*/
...
-- All students that contain the letters "ch" in their name
-- or their names ends with the letters "nd"
...
-- Name of students that have the letters "ae" or "ph"

SELECT * FROM students
WHERE (student_name LIKE '%ae%' OR student_name LIKE '%ph');


-- List of names of students sorted by age descending, largest to smallest
...
-- Names and ages of the top 4 oldest students
...
-- List of students that are not older than 20, if their student_no is either between 3 and 5 or their student_no is 7,
-- and students older than age 20 that have a student_no that is at least 4
...
-- First and last name of employees that have a salary above 140000

SELECT
	first_name || ' ' || last_name AS full_name,
	salary,
	(salary > 140000) AS salary_above_140000
FROM employees;


-- First, last name, and working deparment of employees that have a highly paid salary (above 140000)

SELECT
	first_name || ' ' || last_name AS full_name,
	(salary > 140000) AS is_higly_paid,
	department
FROM employees
WHERE (SALARY > 140000) = 'true';


-- List of departments that have the letters "oth" in between,
-- using a boolean expression to detect if this criteria is true or false
...
-- Selecting and replacing the word 'Clothing' for the word 'Attire'
-- in the departments column

SELECT
	department,
	REPLACE (department,'Clothing','Attire') AS mod_data
FROM departments;


-- Adding the word "department" to every department name

SELECT
	department,
	department || ' ' || 'Department' AS full_department_name
FROM departments;


-- Selecting the position @ in the email column,
-- this will identify the position where the @ is on the email
...
-- List of emails, replacing the null values for non-null values
...
-- Number of total employees, available emails, missing emails,
-- highest salary, lowest salary and average salary
...
-- Salary budget by region_id and department, ordered by region, then department and then salary budget
...
-- List of professors that are considered highly paid if the salary is equal or greater than 95000

SELECT
	last_name,
	salary,
	(salary >= 95000) AS above_or_equal_to_95000,
	'It is ' || (salary >= 95000) || ' that professor ' || last_name || ' is higly paid!' AS conclusion
FROM professors;


-- All the records from the professors table with short department names
-- Version A
SELECT
	last_name,
...
-- Highest and lowest professor's salary, excluding professor Wilson
...
-- Hire date of the professor that has been teaching the longest
...
-- Professor's total working days and years, ordering from highest to lowest working time since the hire date

SELECT
	*,
	(CURRENT_DATE - hire_date) AS total_working_days,
	((CURRENT_DATE - hire_date)/365) AS total_working_years
FROM professors
ORDER BY total_working_years DESC;


-- Total count of employees working for each department, as well as the average, minimum and maximum salary
...
-- Total number of employees, by department and gender
...
-- Departments which have more than 35 employees
...
-- Different domain names and the total number of employees that have that given domain name
...
-- Average, highest and lowest salary per region and by gender*/
...
-- The state with the largest amount of fruit supply
...
-- Most expensive cost_per_unit of every season
...
-- State that has more than 1 import of the same fruit
...
-- Seasons that produce either 3 fruits or 4 fruits
...
-- Most expensive state and its total cost, considering the supply and cost_per_unit columns
...
SELECT
	COUNT(COALESCE(fruit_name,'Fruit'))
FROM fruits;


-- All the departments that exist on the "employees" table and NOT in the "departments" table
...
-- Employees which salary is above 150000, using subqueries
-- Version A
SELECT
	a.employee_name,
	a.yearly_salary
...
FROM (SELECT
	  ...
	  WHERE salary > 150000) AS a,
	 (SELECT
	  	department AS employee_name
	  FROM departments) AS b;
	  

--  All of the employees that work in the electronics division
...
-- Total number of employess that work in the Electronics division
...
-- Employees that work in Asia or Canada, and make over 130000 dollars
-- Version A
...
-- First name and department of employees that work in Asia and Canada, and the difference between their salary
-- and that from the highest paid employee in the company
...
-- All employess that work in the United States
...
-- All employees that are from outside the United States

SELECT * FROM employees
WHERE region_id > ALL (SELECT
					   	region_id
					   FROM regions
					   WHERE country = 'United States');


-- All employees that work in the kids division, and their hire dates is greater than all of the
-- hire dates of employees who work in the maintenance department

SELECT * FROM employees
WHERE department = ANY (SELECT
							department
						FROM departments
						WHERE division = 'Kids')
AND hire_date > ALL (SELECT
					 	hire_date
					 FROM employees
...
-- Average salary, excluding the the highest and the lowest paid employees
...
-- Names of students that are taking the courses "Physics" and "US History"
...
-- Categorizing the salaries and total counts of each category
-- "Underpaid", "Paid Well", and "Executive Paid"
...
-- Transposing the total count of the above categories
...
-- Transposed total count of employees per department
...
-- Fruit total supply, splitting into the following categories:
-- LOW = total supply less than 20,000
-- ENOUGH = total supply between 20,000 and 50,000
-- FULL = total supply is greater than 50,000
...
-- Departments that have more than 38 employees working
-- Version A
...
-- Departments with more than 38 employees working and
-- the highest paid employees salary for each of these departments
...
-- Department and first name of the employees with the
-- overall highest and lowest salary
...
-- Employee's names, email address, division, and country in which the employee,
-- excluding those which email is not available
...
-- Total of employees working in each department and the total of employees
...
-- Employees' first name, division and country they work in, using a VIEW
...
-- Student's name, the courses the student is taking and the professors that teach that course

SELECT student_name, course_no, min(last_name)
FROM (SELECT student_name, student_enrollment.course_no, professors.last_name
	  FROM students
	  ...
	  INNER JOIN professors ON teach.last_name = professors.last_name) a
...
-- All students and any courses they may or may not be taking
...
-- First name, deparment, and number of employees and total salary per department

SELECT
	first_name,
	department,
	COUNT(*) OVER(PARTITION BY department) AS total_depart_employees,
	SUM(salary) OVER(PARTITION BY department)
FROM employees;


-- Number of employees that work in each department as well as the number of employees that work in each region
...
-- Employees first name, department, and count of all that work in region 3
...
-- Employees which rank as "8" on the salary ranking, per department
...
-- Creating buckets of data for the salary ranking, with 1 the highest and 5 the lowest per department
...
-- First value alary of each department
...
-- Showing only the 5th employee salary per deparment, and comparing to the rest of the salaries
-- that follow that 5th employee salary
...
-- Employees first and last name, salary, and comparison with the salary that comes directly before
...
-- Employees first and last name, salary, and comparison with the salary that comes directly next
...
-- Let's now create the sales table and add some values
...
-- Breakdown per continent, country, city and sum of units sold
...
-- Sum of units sold, grouped by continent, country, and city
...
-- Total units sold, breaking down by city, then by country, and, finally by continent
